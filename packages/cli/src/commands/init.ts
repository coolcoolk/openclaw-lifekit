...
export async function initCommand() {
  ...
  const adapterChoice = promptWithDefault("     선택 (1/2/3)", "1");
  const adapterMap: Record<string, string> = { "1": "openclaw", "2": "ollama", "3": "manual" };
  const adapter = adapterMap[adapterChoice] || "openclaw";
  console.log(`     → ${adapter} 선택됨\n`);

  // 6. 구글 캘린더 연동
  console.log("  5️⃣  구글 캘린더 연동");
  ...
  🌐 접속:
     ...
     ${tailscaleIp ? `Tailscale:  http://${tailscaleIp}:5173` : ""}

  📱 모바일 접속:
     ${tailscaleIp ? `http://${tailscaleIp}:5173 을 홈 화면에 추가` : "Tailscale 설정 후 접속 가능"}
  `);
}
